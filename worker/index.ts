// API Worker for outreach-copilot.
//
...
//
// Bindings: DB (D1), AI (Workers AI), AI_MODEL (var) — see types.ts.
// Non-API requests never reach this file: wrangler.jsonc's
// assets.run_worker_first is scoped to "/api/*", so everything else is
// served straight from the built React SPA (see vite.config.ts).
...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    try {
      ...
      return jsonError(new AppError('Internal server error', 500, 'INTERNAL_ERROR'));
    }
  },
